...
QString Stemming::stemmer(QString word)
{
    QString str;
    struct stemmer *z = create_stemmer();

    //char *s = (char*)malloc(sizeof(char)*word.size());
    //printf(s, "%s", word.toStdString().c_str());

    char *s = word.toLatin1().data();

    //s[word.size()-1] = 0;
    //qDebug() << s << "1";

    //s[stem(z, s, word.size()) + 1] = 0;

    int index = stem(z, s, word.size()-1) + 1;
